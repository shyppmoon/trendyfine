        ...
        if ($pay_log)
        {
            ...
            if ($pay_log['order_type'] == PAY_ORDER)
            {
                ...
                if($pay_log['is_paid'] == 0)
				{
					...
					$cart_goods = order_goods($order['order_id']);
				
				
					//付款了给客户发送邮件
					if ($GLOBALS['_CFG']['is_pay_reminded_costome'] && $order['email'] != '')
					{
						$tpl = get_mail_template('pay_reminded_costome');
						$smarty->assign('order', $order);
						$smarty->assign('goods_list', $cart_goods);
						$smarty->assign('shop_name', $GLOBALS['_CFG']['shop_name']);
						$smarty->assign('send_date', date($GLOBALS['_CFG']['time_format']));
						$content = $smarty->fetch('str:' . $tpl['template_content']);
						send_mail($GLOBALS['_CFG']['shop_name'], $order['email'], $tpl['template_subject'], $content, $tpl['is_html']);
					}
					//付款了给商家发送邮件
					if ($GLOBALS['_CFG']['send_service_email'] && $GLOBALS['_CFG']['service_email'] != '')
					{
						$tpl = get_mail_template('is_pay_reminded_service');
						$smarty->assign('order', $order);
						$smarty->assign('goods_list', $cart_goods);
						$smarty->assign('shop_name', $GLOBALS['_CFG']['shop_name']);
						$smarty->assign('send_date', date($GLOBALS['_CFG']['time_format']));
               ...
               $order_info = order_info($order['order_id']);
               $msg = '<div style="color:red; font-size:16px; font-weight:bold">' .  $GLOBALS['_LANG']['pay_success'] . '</div>';
			   $msg .= '<div>Order Number:' .  $order_info['order_sn'] . ',Pay Amount：'. price_format($pay_log['order_amount'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']).'</div></div>';
			   /*
			   if($pay_log['is_paid'] == 0)
			   {
			     $msg .= '<img src="https://shareasale.com/sale.cfm?amount='.price_format($pay_log['order_amount'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']).'&tracking='.$order_info['order_sn'].'&transtype=sale&merchantID=31" width="0" height="0">';
			   }
			   */
			   $GLOBALS['_LANG']['pay_success'] = $msg;

			   echo "【Yan】---In : lib_payment.php, msg=" . $msg;
