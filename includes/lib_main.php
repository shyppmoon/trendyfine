...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1,
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='',$append='')
{
    ...
    if ($pager['styleid'] == 0)
    {
        ...
        {
            $pager['page_first'] = build_uri($app, $uri_args, $append, 1, $keywords);
            $pager['page_prev']  = build_uri($app, $uri_args, $append, $page_prev, $keywords);
            $pager['page_next']  = build_uri($app, $uri_args, $append, $page_next, $keywords);
            $pager['page_last']  = build_uri($app, $uri_args, $append, $page_count, $keywords);
        }
        $pager['array']      = array();
    ...
    else
    {
        ...
        {
            $pager['page_first'] = ($page - $_offset > 1 && $_pagenum < $page_count) ? build_uri($app, $uri_args, $append, 1, $keywords) : '';
            $pager['page_prev']  = ($page > 1) ? build_uri($app, $uri_args, $append, $page_prev, $keywords) : '';
            $pager['page_next']  = ($page < $page_count) ? build_uri($app, $uri_args, $append, $page_next, $keywords) : '';
            $pager['page_last']  = ($_to < $page_count) ? build_uri($app, $uri_args, $append, $page_count, $keywords) : '';
            $pager['page_kbd']  = ($_pagenum < $page_count) ? true : false;
            $pager['page_number'] = array();
            for ($i=$_from;$i<=$_to;++$i)
            {
                $pager['page_number'][$i] = build_uri($app, $uri_args, $append, $i, $keywords);
            }
        }
    }
    if (!empty($sch_array))
...
function assign_template($ctype = '', $catlist = array())
{
	...
	//echo $GLOBALS['_CFG']['integral_name']);
