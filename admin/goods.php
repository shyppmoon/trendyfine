...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
	...
	/*********************************************************************************************/
	/**************************严学文，这里开始计算每个商品查询具体的各个子商品的数量，进行显示***/
	/*********************************************************************************************/
	//print_r($goods_list);													//严学文debug
	$goods_list_YAN = $goods_list["goods"]; 
	foreach($goods_list_YAN as $K_YAN => $v_YAN)
	{
		//print_r($v_YAN);

		$product_YAN = product_list($v_YAN["goods_id"], '');				//读取每个商品id的相关货品列表。

		$goods_list["goods"][$K_YAN]["YAN_this_p_products"]	= $product_YAN; //把这个商品的product货品信息插入到这个商品list信息里面去，如下图示
		$goods_list["goods"][$K_YAN]["goods_weight"]	= 	$goods_list["goods"][$K_YAN]["goods_weight"] * 1000;

		//计算价格 = (采购价*1.4+物流费)/0.95/6.1, 赚的毛利按照采购价的40%计算，如果少于20则取20，如果超过50则取50
		//【说明】：这个earnmoney的计算总共在三个地方需要用到：1、在文件夹 admin/index.php文件里：piliang_gai_jiage 时 + 2、admin/goods.php文件的act=list 及 act=query 产品列表页面产品查询页面的时候分别用到这个赚钱的计算公式，请注意 

		$yan_buy_price = $goods_list["goods"][$K_YAN]["goods_buy_price"];
		if($yan_buy_price<40)					    //0-40采购价，毛利设定为采购价的50%
		{
			$earnmony = $yan_buy_price*0.5;
		}
		else if($yan_buy_price<50)					//40-50采购价，毛利设定为采购价的45%
		{
			$earnmony = $yan_buy_price*0.5;
		}
		else if($yan_buy_price<120)					//50-120采购价，毛利设定为采购价的40%
		{
			$earnmony = $yan_buy_price*0.5;
		}
		else										//120以上采购价，毛利设定为固定的50元。
		{
			//$earnmony = 50;
			$earnmony = $yan_buy_price*0.4;
		}

		$goods_list["goods"][$K_YAN]["yan_price"] = round( ($goods_list["goods"][$K_YAN]["goods_buy_price"] + $earnmony + $goods_list["goods"][$K_YAN]["goods_weight"]/1000*90 +8)/0.95/6.1, 2);

		$goods_list["goods"][$K_YAN]["yan_chengben"] = round( ($goods_list["goods"][$K_YAN]["goods_buy_price"] + $goods_list["goods"][$K_YAN]["goods_weight"]/1000*90 +8)/0.95, 2);

		$goods_list["goods"][$K_YAN]["yan_earnmony"] = $earnmony;
		$goods_list["goods"][$K_YAN]["yan_shipping"] = round( ($goods_list["goods"][$K_YAN]["goods_weight"]/1000*90 +8)/0.95, 2); //运费

		$goods_list["goods"][$K_YAN]["yan_percent"] = round($earnmony/6/$goods_list["goods"][$K_YAN]["yan_price"] , 2)*100; //毛利百分比
				...
				(
					[0] => Array
						(
							...
							[integral] => 0
							[is_promote] => 0
	==>严学文新增项目		[YAN_this_p_products] => Array(这个是product货品的格式，具体请在lib_goods.php文件的product_list函数所返回的格式)
						)
    ...
    else
    {
        ...
        if ($_REQUEST['act'] == 'copy')
        {
           ...
           $goods['original_img'] = '';
			...
			if ($is_copy)
			{
				$smarty->assign('copy_goods_id', $_REQUEST['goods_id']);
			}
			...
			{
			    $goods['original_img'] = reformat_image_name_copy('goods', $_REQUEST['goods_id'], $goods['original_img'], 'source');
			}
			if(!empty($goods['goods_img']))
			{
			    $goods['goods_img'] = reformat_image_name_copy('goods', $_REQUEST['goods_id'], $goods['goods_img'], 'goods');
			}
			if(!empty($goods['goods_thumb']))
			{
			   $goods['goods_thumb'] = reformat_image_name_copy('goods_thumb', $_REQUEST['goods_id'], $goods['goods_thumb'], 'thumb');
			}*/
			/*wzys复制商品时，将货品信息和图片一起复制end*/
        ...
        /* 图片列表 */
        $sql = "SELECT * FROM " . $ecs->table('goods_gallery') . " WHERE goods_id = '$_REQUEST[goods_id]'";
		...
		if ($is_copy)
		{
			foreach($img_list as $key=>$val)
			{
				...
				{
					$img_list[$key]['img_url'] = reformat_image_name_copy('gallery', $_REQUEST['goods_id'], $val['img_url'], 'goods');
				}
				if(!empty($val['thumb_url']))
				{
					$img_list[$key]['thumb_url'] = reformat_image_name_copy('gallery_thumb', $_REQUEST['goods_id'], $val['thumb_url'], 'thumb');
				}
				if(!empty($val['img_original']))
				{
				   $img_list[$key]['img_original'] = reformat_image_name_copy('gallery', $_REQUEST['goods_id'], $val['img_original'], 'source');
				}
			}
		}
		*/
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $_POST['goods_name'] = ucwords($_POST['goods_name']);
    /* 入库 */
    if ($is_insert)
    {
        if ($code == '')
	...
	/* 在商品的goods数据表中添加字段，然后在goods_info.htm文件里面添加模板input输入框，这个地方添加php执行语句把数据保存进入到表格中去*/
	$sql = "UPDATE " .$ecs->table('goods'). " SET goods_sn1 = '$_POST[goods_sn1]' WHERE goods_id = '$goods_id' LIMIT 1";
    $db->query($sql);
	$sql = "UPDATE " .$ecs->table('goods'). " SET goods_buy_price = '$_POST[goods_buy_price]' WHERE goods_id = '$goods_id' LIMIT 1";
    $db->query($sql);
	$sql = "UPDATE " .$ecs->table('goods'). " SET goods_weight_review = '$_POST[goods_weight_review]' WHERE goods_id = '$goods_id' LIMIT 1";
    $db->query($sql);

    /*wzys复制商品时，将货品信息和图片一起复制*/
	/*$sql = "insert into  " .$ecs->table('goods'). "(`goods_id`, `goods_attr`, `product_sn`, `product_number`) select`goods_id`, `goods_attr`, `product_sn`, `product_number` from " .$ecs->table('goods'). " where goods_id="*/
	/*wzys复制商品时，将货品信息和图片一起复制end*/
    ...
    /*--wzys修改过代码--*/
	$img_original_a = isset($_POST['img_original']) ? $_POST['img_original'] : array();
    foreach($img_original_a as $key=>$val)
	{
		$img_original = $val;
		$img_url = $_POST['img_url'][$key];
		$thumb_url = $_POST['thumb_url'][$key];
		
		$sql = "INSERT INTO " . $ecs->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
                "VALUES ('$goods_id', '$img_url', '', '$thumb_url', '$img_original')";
        $db->query($sql);
	}
	/*--wzys修改过代码 end--*/
    /* 处理相册图片 */
...
elseif ($_REQUEST['act'] == 'edit_goods_weight')
{
    ...
    $goods_id       = intval($_POST['id']);
    $goods_weight     = intval($_POST['val'])/1000;
...
//-- 修改商品重量  ---- 严学文新增
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_goods_weight_review')
{
    check_authz_json('goods_manage');

    $goods_id       = intval($_POST['id']);
    $goods_weight_review     = intval($_POST['val']);

    if ($exc->edit("goods_weight_review = '$goods_weight_review', last_update=" .gmtime(), $goods_id))
    {
        clear_cache_files();
        make_json_result($goods_weight_review);
    }
}
...
elseif ($_REQUEST['act'] == 'query')
{
	...
	foreach($goods_list_YAN as $K_YAN => $v_YAN)
	{
		...
		$goods_list["goods"][$K_YAN]["yan_price"]	= round( ($goods_list["goods"][$K_YAN]["goods_buy_price"] + $earnmony + $goods_list["goods"][$K_YAN]["goods_weight"]/1000*90 +8)/0.95/6.1, 1);
