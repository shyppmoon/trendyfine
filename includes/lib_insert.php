...
/*wzys多国货币*/
function insert_select_currency()
{
     $sql  = 'SELECT * '.
               ' FROM ' .$GLOBALS['ecs']->table('currency').
                " where is_show=1 order by sort_order asc";
	$directory_currency = $GLOBALS['db']->getAll($sql);
	
	$max_key = count($directory_currency);
	$directory_currency[$max_key]['currency_name'] = $GLOBALS['_CFG']['currency_name'];
	$directory_currency[$max_key]['currency_icon'] = $GLOBALS['_CFG']['currency_to'].'.jpg';
	$directory_currency[$max_key]['currency_to'] = $GLOBALS['_CFG']['currency_to'];
	$directory_currency[$max_key]['currency_format'] = $GLOBALS['_CFG']['currency_format'];
	$directory_currency[$max_key]['rate'] = 1.00;
	
	$GLOBALS['smarty']->assign('currency_list',     $directory_currency);
	
	
	$val = $GLOBALS['smarty']->fetch('library/currency.lbi');
    return $val;
}
/*wzys多国货币*/
?>