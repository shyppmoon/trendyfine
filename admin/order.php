...
elseif ($_REQUEST['act'] == 'info')
{
	...
	$order['country_name']  = $db->getOne("SELECT region_name as country_name from " . $GLOBALS['ecs']->table('region') . " WHERE region_id = '$order[country]'");
	$order['province_name']  = $db->getOne("SELECT region_name as province_name from " . $GLOBALS['ecs']->table('region') . " WHERE region_id = '$order[province]'");

$order['billing_country_name']  = $db->getOne("SELECT region_name as country_name from " . $GLOBALS['ecs']->table('region') . " WHERE region_id = '$order[billing_country]'");
	$order['billing_province_name']  = $db->getOne("SELECT region_name as province_name from " . $GLOBALS['ecs']->table('region') . " WHERE region_id = '$order[billing_province]'");
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        $row['formated_subtotal']       = price_format($row['goods_price'] * $row['goods_number'],true,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $row['formated_goods_price']    = price_format($row['goods_price'],true,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    /* 取得参数 step */
    $step_list = array('user', 'edit_goods', 'add_goods', 'goods', 'consignee1', 'consignee', 'shipping', 'payment', 'other', 'money', 'invoice');
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    /* 取得参数 step */
    $step_list = array('user', 'goods', 'consignee', 'consignee1', 'shipping', 'payment', 'other', 'money');
...
elseif ($_REQUEST['act'] == 'get_goods_info')
{
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        $row['formated_subtotal']       = price_format($row['goods_price'] * $row['goods_number'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $row['formated_goods_price']    = price_format($row['goods_price'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                    ...
                    "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer,curr_to,curr_rate , curr_format ".
    ...
    {
        $row[$key]['formated_order_amount'] = price_format($value['order_amount'],true,true,$value['curr_to'],$value['curr_rate'],$value['curr_format']);
        $row[$key]['formated_money_paid'] = price_format($value['money_paid'],true,true,$value['curr_to'],$value['curr_rate'],$value['curr_format']);
        $row[$key]['formated_total_fee'] = price_format($value['total_fee'],true,true,$value['curr_to'],$value['curr_rate'],$value['curr_format']);
