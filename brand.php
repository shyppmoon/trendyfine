...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update'))) ? trim($_REQUEST['sort'])  : (isset($_COOKIE['ECS']['sort']) ? $_COOKIE['ECS']['sort'] : $default_sort_order_type);
$order = (isset($_REQUEST['order']) && in_array(trim(strtoupper($_REQUEST['order'])), array('ASC', 'DESC')))                              ? trim($_REQUEST['order']) : (isset($_COOKIE['ECS']['order']) ? $_COOKIE['ECS']['order'] : $default_sort_order_method);


setcookie('ECS[sort]', $sort, gmtime() + 86400 * 7,'/');
setcookie('ECS[order]', $order, gmtime() + 86400 * 7,'/');
...
if (!$smarty->is_cached('brand.dwt', $cache_id))
{
   ...
   /*--wzys修改过代码--*/
	if(!empty($brand_info['custome_title']))
	{
		$position['title'] = $brand_info['custome_title'];
		$smarty->assign('page_title',       $position['title']);    // 页面标题
	}
	else
	{
