...
class yeepay_jcard
{
    ...
    function respond()
    {
        ...
        $merchant_id    = $payment['yp_account'];       // 获取商户編號
        ...
        $amount         = trim($_REQUEST['r3_Amt']);    // 获取訂單金额
        $cur            = trim($_REQUEST['r4_Cur']);    // 获取訂單货币单位
        $product_id     = trim($_REQUEST['r5_Pid']);    // 获取产品ID
        $orderid        = trim($_REQUEST['r6_Order']);  // 获取訂單ID
        $userId         = trim($_REQUEST['r7_Uid']);    // 获取产品ID
        $merchant_param = trim($_REQUEST['r8_MP']);     // 获取商户私有参数
        $bType          = trim($_REQUEST['r9_BType']);  // 获取訂單ID
