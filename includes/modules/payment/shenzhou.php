...
class shenzhou
{
   ...
   /**
    ...
    function get_code($order, $payment)
    {
        ...
        $payer_contact_type = '';
        $payer_contact      = '';
        $order_id           = $order['order_sn'];                                //商户訂單号 不可空
        $order_amount       = $order['order_amount'] * 100;                    //商户訂單金额 不可空
        $pay_type           = '00';                                            //支付方式 不可空
        $card_number        = '';
        $card_pwd           = '';
        $full_amount_flag   = '0';
        $order_time         = local_date('YmdHis', $order['add_time']);        //商户訂單提交时间 不可空 14位
        $product_name       = '';
        $product_num        = '';
        $product_id         = '';
        $product_desc       = '';
    ...
    function respond()
    {
        ...
        $order_id            = trim($_REQUEST['orderId']);            //訂單号
        ...
        if (strtoupper($sign_msg) == strtoupper($merchant_signmsg))
        {
            ...
            elseif ($pay_result == 11  && $pay_amount > 0)
            {
                ...
                if ($get_order_amount == $pay_amount && $get_order_amount == $order_amount) //检查訂單金额、实际支付金额和訂單是否相等
                {
                    order_paid($ext1);

                    return true;
                }
                elseif ($get_order_amount == $order_amount && $pay_amount > 0) //訂單金额相等 实际支付金额 > 0的情况
                {
                    $surplus_amount = $get_order_amount - $pay_amount;        //计算訂單剩余金额
