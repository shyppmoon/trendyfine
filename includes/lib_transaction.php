...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time, " .
           "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee,curr_to,curr_rate,curr_format ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'order_status'   => $row['order_status'],
					    /*--wzys修改过代码--*/
                       'total_fee'      => price_format($row['total_fee'],false,true,$row['curr_to'],$row['curr_rate'],$row['curr_format']),
					   /*--wzys修改过代码 end--*/
                       'handler'        => $row['handler']);
    }
