...
/* 严学文修改默认排序方式为：根据排序、goods_id的降序来进行 */
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
$sort = "sort_order DESC,".$sort;		//默认情况下把序号降序排序放进去---也就是默认情况下是管理员推荐的，以后遇到多种不同的排序方式的时候要修改
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
	...
	$all_url = build_uri('category', array('cid'=>$cat_id),$cat['cat_name']);	//该分类根目录
	$smarty->assign('all_url',  $all_url);										//该分类根目录	

	/*******************************************************************************************************************************/
	/*****   处理筛选的时候，取消筛选时返回的URL，及显示筛选项目的 属性名称attr_value  筛选串为：$filter_attr_str      *************/
	/*******************************************************************************************************************************/
	$flag_filter_attr_link = 0;											//标记是否该分类中有属性被选中？ 用这个标记控制分类标题+筛选属性后标题
	$arr_attr_bread_cancel = array();
	$filter_attr_link = $filter_attr;									//用于下面拼接新的url使用，
	if($filter_attr_str != '' and $filter_attr_str != "0")
	{
		$flag_filter_attr_link = 1;										//有属性被选中
		foreach($filter_attr as $k_YAN => $v_YAN)
		{
			if($v_YAN > 0)	   //跳过那些本身就不选择的属性
			{
				...
				$attr_cancel = trim($url_temp_link,'-');					//去掉串中的第一个字符“-”。

				$flag_arr_cancel = 0;
				$arr_cancel	= explode('-', $attr_cancel);
				foreach($arr_cancel as $kc => $vc)
				{
					if($vc > 0)	//判断取消后的网址是不是全部变成”0-0-0-0-0-“的格式，如果是这样的格式那么就直接回归到如”category-32-Apparel.html“去
					{
						$flag_arr_cancel = 1;
					}
				}
				if($flag_arr_cancel == 1)
				{
					$arr_attr_bread_cancel[$k_YAN]["url"] = build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$attr_cancel), $cat['cat_name']);
				}
				else   
				{
					$arr_attr_bread_cancel[$k_YAN]["url"] = $all_url;//该分类根目录build_uri('category',array('cid'=>$cat_id),$cat['cat_name']);
				}
	...
	//print_r($attr_list);
                ...
                foreach ($attr_list as $k => $v)
                {
				   ...
				   //echo "<br>";
					...
					$all_attr_list[$key]['attr_list'][$temp_key]['goods_num'] = $count_YAN;

					/**************************************************************************************************************/
					/**************************************************************************************************************/
					/**************************************************************************************************************/
					//if($count_YAN>0)		//在下文的整理中会去掉数量=0的情况，这里可以略过判断。开始统计数量>0的情况才把链接记录进去list中去
					{						//$all_attr_list[$key]['attr_list'][$temp_key]['attr_value'] = $v['attr_value'];
						...
						if($count_YAN>1)    //当数量>1的时候，正常组织网址URL
						{
						...
						else				//当数量=1且无筛选的时候,设定这个URL为空，同时模板中不显示URL，只显示属性名和数量，请参看shaixuan.lbi文件
						{
							if($flag_filter_attr_link == 0 && $count_YAN == 1)//当数量=1且无选择flag=0的时候,就是无属性选择的分类页,也需要组织URL
							{
								$all_attr_list[$key]['attr_list'][$temp_key]['url'] = build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url), $cat['cat_name']);
							}
							else			//数量=0或者有筛选以后且数量=1的情况，都设定URL为空""
							{
								$all_attr_list[$key]['attr_list'][$temp_key]['url'] = "";
							}
						}
						...
						if (!empty($filter_attr[$key]) AND $filter_attr[$key] == $v['goods_id'])
						{
							...
							//echo "识别什么时候是选中！！";
							$selected_attr_names_link = $selected_attr_names_link . " " . $selected_attr_names_link_temp;
		...
		/**************************************************************************************************************/
		/**************************************************************************************************************/
		/*******这里是去重新整理属性list，当某一行的某个属性已经selected，那么这一行的其他兄弟属性需要去掉不显示*******/
		//echo "<br>严学文debug All_attr_list<br>";
		//print_r($all_attr_list);
		$arr_attr_list_YAN = $all_attr_list;
		$arr_attr_list_YAN_used = array();
		foreach ($arr_attr_list_YAN as $k_YAN => $v_YAN)
		{
			$this_big_cat_attr = $v_YAN["attr_list"];
			$new_this_big_selectd_flag = 0;

			$new_this_big_cat_attr = array();
			$new_this_big_selectd_position = 0;
			foreach ($this_big_cat_attr as $k_YAN_2 => $v_YAN_2)	//循环这个大类属性内部的所有属性
			{
				if($k_YAN_2 != 0 && $v_YAN_2["selected"])
				{	  												//echo "<br>！！！这一排有被选择的子属性。";
					$new_this_big_selectd_flag = 1;					//设置标记为1，标明该大类存在被选择的子属性
					$new_this_big_cat_attr = $v_YAN_2;				//记录这个时候的arr值。
					$new_this_big_selectd_position = $k_YAN_2;		//记录这个selected子属性的id位置  //print_r($new_this_big_cat_attr);
				} 
			}

			if($new_this_big_selectd_flag == 1)						//这个大类中存在子属性被选中的情况
			{
			  $v_YAN_temp = array();
			  $v_YAN_temp["filter_attr_name"] = $v_YAN["filter_attr_name"];	 //记录本次大属性的名称: Color, Style ???
			  $v_YAN_temp["attr_list"][0] = $this_big_cat_attr[0];	//把第一项，“全部”的这1项加入
			  $v_YAN_temp["attr_list"][1] = $this_big_cat_attr[$new_this_big_selectd_position];	   //把选中的那个子属性的赋值
			  $arr_attr_list_YAN_used[$k_YAN] = $v_YAN_temp;		//有选择，把选择后的重组后的值取代原来的数组
			}
			else
			{
				$arr_attr_list_YAN_used[$k_YAN] = $v_YAN;			//无选择，一切都这样执行
			}
		}															//echo "<br>严学文整理后！！！";   	//print_r($arr_attr_list_YAN_used);
		$all_attr_list =  $arr_attr_list_YAN_used;					//使用最新的这个属性链接list
		/**************************************************************************************************************/
		/**************************************************************************************************************/
		/**************************************************************************************************************/
		//print_r($all_attr_list); 
		...
		{
		   unset($all_attr_list[$key]['attr_list'][0]);		   //删除ecshop原生态代码中“全部"的选项，数组第"0"个元素
		}
		foreach($all_attr_list as $key=>$val)
		{
			foreach($val['attr_list'] as $k=>$v)
			{
				if($v['goods_num']==0)						  //删除数量=0的那个属性在筛选中的显示，不允许显示
				{
	...
	//以下进行分类筛选页面的标题和描述组合信息设置
	if($flag_filter_attr_link == 1)	    //有属性被选中时，参数$selected_attr_names_link是所有组合得到的字符串
	{
		$selected_attr_names_link = trim($selected_attr_names_link);	//去掉前后无效空格
		$title_for_this_category = $selected_attr_names_link . " " . $cat['cat_short_title'];	//这是有属性选中情况下组建title
				
		$description_for_this_category = $cat['cat_short_desc']. " " .$selected_attr_names_link. " " .$cat['cat_short_desc1']. " " .$selected_attr_names_link. " " .$cat['cat_short_desc2']. " " .$selected_attr_names_link. " " .$cat['cat_short_desc3'];			//这是在属性选中情况下组建description
	}
	else								//目前无属性被选中
	{
		$title_for_this_category = $cat['cat_title'];							//这是无属性选中情况下，标题
		$description_for_this_category = $cat['cat_desc'];					//这是在属性选中情况下，描述
	}
	
    $smarty->assign('page_title',       $title_for_this_category);				//页面标题，原来是$position['title']
	$smarty->assign('page_description', $description_for_this_category);		//页面描述
	$smarty->assign('flag_filter_attr_link',       $flag_filter_attr_link);				//是否有筛选的标志
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, cat_title, cat_short_title,cat_short_desc,cat_short_desc1,cat_short_desc2,cat_short_desc3, keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
