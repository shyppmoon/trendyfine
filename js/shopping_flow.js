...
{
  document.getElementById('cart_goods').innerHTML=obj.cart_goods;
  if (obj.error)
  {
    alert(obj.error);
    orderSelectedResponse(obj.content);
...
function checkOrderForm(frm)
{
  ...
  for (i = 0; i < frm.elements.length; i ++ )
  {
	...
	if (frm.elements[i].name == 'billing_method' && frm.elements[i].checked)
	{
		billing_method_select = frm.elements[i].value;
	}
  }
  
  
  
  if(billing_method_select==2)
  {
	  if (Utils.isEmpty(frm.elements['billing_firstname'].value))
	  {
		err = true;
		msg.push("Please input your first name!");
	  }
	  if (Utils.isEmpty(frm.elements['billing_lastname'].value))
	  {
		err = true;
		msg.push("Please input your last name!");
	  }
	
	  if (frm.elements['billing_country'] && frm.elements['billing_country'].value == 0)
	  {
		msg.push("Please select a country!");
		err = true;
	  }
	
	  if (frm.elements['billing_province'] && Utils.isEmpty(frm.elements['billing_province'].value))
	  {
		err = true;
		msg.push("Please select a province/state!");
	  }
	  if (Utils.isEmpty(frm.elements['billing_city'].value))
	  {
		err = true;
		msg.push("Please input your city!");
	  }
	  if (frm.elements['billing_address'] && Utils.isEmpty(frm.elements['billing_address'].value))
	  {
		err = true;
		msg.push("Please input your Address Line 1");
	  }
	  if (Utils.isEmpty(frm.elements['billing_zipcode'].value))
	  {
		err = true;
		msg.push("Please input your Post/Zip Code!");
	  }
	  if (Utils.isEmpty(frm.elements['billing_tel'].value))
	  {
		err = true;
		msg.push("Please input your Telephone!");
	  }
	  
	  if (!Utils.isEmail(frm.elements['billing_email'].value))
	  {
		err = true;
...
function checkConsignee(frm)
{
  ...
  if (frm.elements['province'] && Utils.isEmpty(frm.elements['province'].value))
  {
    err = true;
    msg.push("Please select or input a State/Province!");
  }
  ...
  for (i = 0; i < frm.elements.length; i ++ )
  {
   	if (frm.elements[i].name == 'is_create_account' && frm.elements[i].checked)
	{
		is_create_account = frm.elements[i].value;
	}
  }
  ...
  {
	  if (Utils.isEmpty(frm.elements['email_register'].value)) {
		err = true;
        msg.push("Is your email address correct? Please try again.");
	  }

	  if (!Utils.isEmail(frm.elements['email_register'].value)) {
	  ...
	  if (Utils.isEmpty(frm.elements['password'].value)) {
		err = true;
        msg.push(password_not_null);
		
	  }
	  if (frm.elements['password'].value.length < 6) 
	  {
		err = true;
        msg.push(password_lt_six);
	  }
	  if (frm.elements['password'].value != frm.elements['confirm_password'].value) {
		err = true;
        msg.push(password_not_same);
	  }
  }
