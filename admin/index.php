...
elseif ($_REQUEST['act'] == 'piliang_gai_jiage')
{
	$sql = "select goods_name,goods_id,goods_weight,goods_buy_price from " .$ecs->table('goods'). "";
    $goods_list = $db->getAll($sql);
	
	foreach($goods_list as $key=>$val)
	{
		//批量修改产品标题。。。。把单词的第一个字母大写
		//$goods_name = addslashes(ucwords($val['goods_name']));
		//$sql = "update " .$ecs->table('goods'). " set goods_name='$goods_name' where goods_id=$val[goods_id]";
		//$db->query($sql); 
		...
		if($val["goods_buy_price"]<40)					   //0-40采购价，毛利设定为采购价的50%
		{
			$earnmony = $val["goods_buy_price"]*0.5;
		}
		else if($val["goods_buy_price"]<50)					//40-50采购价，毛利设定为采购价的45%
		{
			$earnmony = $val["goods_buy_price"]*0.5;
		}
		else if($val["goods_buy_price"]<120)			    //50-120采购价，毛利设定为采购价的40%
		{
			$earnmony = $val["goods_buy_price"]*0.5;
		}
		...
		{
			//$earnmony = 50;
			$earnmony = $val["goods_buy_price"]*0.4;
		}

		$shop_price   = round( ($val["goods_buy_price"] + $earnmony + $val["goods_weight"]*90 +8)/0.95/6.1, 2);
		$market_price = round( ($val["goods_buy_price"] + $earnmony + $val["goods_weight"]*90 +8)/0.95/6.1*1.2, 2);	  //即为售价=83折，优惠17% OFF
		$sql = "update " .$ecs->table('goods'). " set shop_price='$shop_price',market_price='$market_price' where goods_id=$val[goods_id]";
		$db->query($sql);
	}
...
elseif ($_REQUEST['act'] == 'copy_goods_wi')	  //从tfd_goods复制数据到tfd_goods_copy表格，这个表格多了1个字段：img_other_urls_forcopy
{
	//首先清空tfd_goods_copy里面的老数据
	$sql = "TRUNCATE TABLE " .$ecs->table('goods_copy'). "";
	$db->query($sql);

	$sql = "select * from " .$ecs->table('goods'). " where is_on_sale=1 order by goods_id ASC";	//只复制还在销售的商品	 
    $goods_list = $db->getAll($sql);
	
	foreach($goods_list as $key=>$val)				  //循环所有的数据，逐个进行处理
	{
		//拼接该产品的所有细节图片url
		$img_other_urls_forcopy = "";
		$sql_gallery = "select * from ". $ecs->table('goods_gallery') . " where goods_id=" . $val["goods_id"];
		$goods_gallery_list = $db->getAll($sql_gallery);
		$cnt = 0;
		foreach($goods_gallery_list as $key_ga=>$val_ga)
		{
			if($cnt == 0)
			{
				$img_other_urls_forcopy = $val_ga["img_original"];
			}
			else
			{
				$img_other_urls_forcopy = $img_other_urls_forcopy . "," . $val_ga["img_original"];	
			}
			$cnt++;
		}
		
		//计算这个产品所有的尺码：如SML，然后每个尺码产生一条记录。goods_sml_size 是商品的货品信息, 也就是尺寸信息。
		$goods_sml_size = "";
		$sql_product= "select * from ". $ecs->table('products') . " where goods_id=" . $val["goods_id"] . " and product_number>0";//有货的size才导出
		$goods_product_list = $db->getAll($sql_product);
		foreach($goods_product_list as $key_product=>$val_product)
		{
			//把product里面的size信息去掉商品货号，得到纯粹的SML信息
			$goods_sml_size = str_replace($val["goods_sn"]."-", "", $val_product["product_sn"]); 

			//为每个Size创建一个记录
			$sql_insert = "INSERT INTO " .$ecs->table('goods_copy').
                " (goods_id,goods_id_real,cat_id,goods_sn,goods_sml_size,goods_name,goods_weight,market_price,shop_price,goods_desc,goods_thumb,goods_img,original_img,is_on_sale,goods_buy_price,img_other_urls_forcopy,goods_weight_review) ".
                "VALUES" . " ('','" .$val["goods_id"]. "', '" .$val["cat_id"]. "', '" .$val["goods_sn"]. "','" .$goods_sml_size. "', '" .addslashes($val["goods_name"]). "', '" .$val["goods_weight"]. "', '" .$val["market_price"]. "', '" .$val["shop_price"]. "', '" .addslashes($val["goods_desc"]). "', '" .$val["goods_thumb"]. "', '" .$val["goods_img"]. "', '" .$val["original_img"]. "', '" .$val["is_on_sale"]. "', '" .$val["goods_buy_price"]. "', '" .$img_other_urls_forcopy. "', '" .$val["goods_weight_review"]. "')";
			$db->query($sql_insert); 
		}	
	}
	
    sys_msg("批量复制数据成功，请联系严导出excel数据。");
}
