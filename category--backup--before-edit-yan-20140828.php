...
$smarty->assign('all_url',  build_uri('category', array('cid'=>$cat_id),$cat['cat_name']));
    ...
    if ($cat['filter_attr'] > 0)
    {
        ...
        {
			//echo "<br>";
           $sql = "SELECT a.attr_name FROM " . $ecs->table('attribute') . " AS a, " . $ecs->table('goods_attr') . " AS ga, " . $ecs->table('goods') . " AS g WHERE ($children OR " . get_extension_goods($children) . ") AND a.attr_id = ga.attr_id AND g.goods_id = ga.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND a.attr_id='$value' $ext";
            ...
            if($temp_name = $db->getOne($sql))
            {
               ...
               $all_attr_list[$key]['filter_attr_name'] = $temp_name;
                ...
                $sql = "SELECT a.attr_id, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value,COUNT(*) AS goods_num FROM " . $ecs->table('goods_attr') . " AS a, " . $ecs->table('goods') .
                       " AS g" .
	...
	//print_r($attr_list);
                ...
                foreach ($attr_list as $k => $v)
                {
				   ...
				   //echo "<br>";
					...
					$all_attr_list[$key]['attr_list'][$temp_key]['goods_num'] = $v['goods_num'];
	
	$yxw_addto_title_temp = $v['attr_value'];	   //严学文拼接属性名称到标题
	//print_r($v);
	//echo "<br>";
                    ...
                    { //由于技术员在这个双重循环中的sql语句跟原版程序中的sql语句不同，所以这个地方判断是否属性选中是错误的，请切记！！！
			
					/*  $attr_list_selected[$key]['attr_value'] = $v['attr_value'];
					  $attr_list_selected[$key]['url'] =  build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url2,'comment_rank'=>$comment_rank), $cat['cat_name']);*/
			
						$yxw_addto_title .=  ", " . $yxw_addto_title_temp;				//严学文拼接属性名称到标题
	 ...
	 /*--wzys ecshop前台属性筛选带删除功能　颜色属性带图片展示修改过代码--*/	
        $smarty->assign('attr_list_selected', $attr_list_selected);
	 /*--wzys ecshop前台属性筛选带删除功能　颜色属性带图片展示修改过代码end--*/
