 ...
 * @return  string
 */
function price_format($price, $change_price = true, $is_format=true, $curr_to='',$curr_rate=0,$curr_format='')
{
	...
	elseif(!empty($GLOBALS['currency_rate']))
	{
		$rate = $GLOBALS['currency_rate'];
	}
	...
	$curr_format= trim($curr_format);
	if(!empty($curr_format))
	{
		$currency_format = $curr_format;
	}
	elseif(!empty($GLOBALS['currency_format']))
	{
		$currency_format = $GLOBALS['currency_format'];
	}
	else
	{
		$currency_format = $GLOBALS['_CFG']['currency_format'];
	}
...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    ...
    switch ($app)
    {
        case 'category':
            ...
            else
            {
                if ($rewrite)
                {
                    $uri = 'category-' . $cid;		   //原来是 $uri = 'category-' . $cid;
					if (!empty($page))
                    {
                        $uri .= '-p' . $page;
                    }
    ...
    if ($rewrite)
    {
        ...
        {
            //$uri .=  '-'.urlencode(preg_replace('/[\.|\/|\?|&|\+|\\\|\'|"|,\s]+/', '-', $append));
			$uri .=  '-'.urlencode(preg_replace('/[\.|\/|\?|&|\+|\\\|\'|"|,\s|\(|\)|@|#|$|*|%|\^|=|~|!|<|>|\[|\]|{|}|:|\'|"|;]+/', '-', $append));
        }
        $uri .= '.html';
    }
...
function send_register_bonus($user_id,$bid) 
{
	$user_info = $GLOBALS['db']->getRow("select user_name, email from " . $GLOBALS['ecs']->table('users') . "where user_id ='$user_id' " );
	$bonus_type = $GLOBALS['db']->getRow("SELECT * FROM " . $GLOBALS['ecs']->table('bonus_type') . " WHERE type_id = '$bid'");
	...
	$sql = "INSERT INTO " . $GLOBALS['ecs']->table('email_sendlist') . " ( email, template_id, email_content, pri, last_send) VALUES ('$user_info[email]', $template_id, '$content', 1, '$time')";
	$GLOBALS['db']->query($sql);
	/* 向会员红包表录入数据*/
