...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    if (empty($goods->spec) AND empty($goods->quick))
    {
        ...
        "WHERE a.comstome_type = 1 AND g.goods_id = '" . $goods->goods_id . "' " .
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    else
    {
        ...
        $consignee = array(
            ...
            'firstname'     => empty($_POST['firstname'])  ? '' :   compile_str(trim($_POST['firstname'])),
			'lastname'     => empty($_POST['lastname'])  ? '' :   compile_str(trim($_POST['lastname'])),
			...
			'address_2'       => empty($_POST['address_2'])    ? '' :   compile_str($_POST['address_2']),
		...
		{
			$is_create_account    = empty($_POST['is_create_account']) ? 0 : intval($_POST['is_create_account']);
			if($is_create_account==1)
			{
				include_once('includes/lib_passport.php');
				$username = isset($_POST['email_register']) ? trim($_POST['email_register']) : '';
				$email = trim($_POST['email_register']);
				$password = trim($_POST['password']);
			    
				if (empty($email))
				{
					show_message($GLOBALS['_LANG']['email_empty']);
				}
				else
				{
					if (!is_email($email))
					{
						show_message(sprintf($GLOBALS['_LANG']['email_invalid'], htmlspecialchars($email)));
					}
				}
				...
				{
					show_message(sprintf($GLOBALS['_LANG']['email_exist'], $email));
				}
				
				if (register($username, trim($_POST['password']), trim($_POST['email_register'])))
				{
					if ($_CFG['is_send_reg'] && $email != '')
					{
...
elseif ($_REQUEST['step'] == 'checkout')
{
	...
	$consignee['country_name']  = $db->getOne("SELECT region_name as country_name from " . $GLOBALS['ecs']->table('region') . " WHERE region_id = '$consignee[country]'");
	//$consignee['province_name']  = $db->getOne("SELECT region_name as province_name from " . $GLOBALS['ecs']->table('region') . " WHERE region_id = '$consignee[province]'");
/*	$order['city_name']  = $db->getOne("SELECT region_name as city_name from " . $GLOBALS['ecs']->table('region') . " WHERE region_id = '$order[city]'");
	$order['district_name']  = $db->getOne("SELECT region_name as district_name from " . $GLOBALS['ecs']->table('region') . " WHERE region_id = '$order[district]'");
*/
$consignee['province_name'] = $consignee['province'];
	...
	{
		$sql = 'SELECT * FROM ' . $ecs->table('region') . " WHERE `region_id` = " . $consignee['country'];		//获取国家
		$temp_yan = $GLOBALS['db']->getAll($sql);
		$consignee['country_name'] = $temp_yan[0]['region_name'];
	}
	else
	{
	...
	{
		$sql = 'SELECT * FROM ' . $ecs->table('region') . " WHERE `region_id` = " . $consignee['province'];
		$temp_yan = $GLOBALS['db']->getAll($sql);
		$consignee['province_name'] = $temp_yan[0]['region_name'];
	}
	else
	{
	...
	if($consignee['city']!='')
	{
		$sql = 'SELECT * FROM ' . $ecs->table('region') . " WHERE `region_id` = " . $consignee['city'];
		$temp_yan = $GLOBALS['db']->getAll($sql);
		$consignee['city_name'] = $temp_yan[0]['region_name'];
	}
	else
	{
		$consignee['city_name'] = '';
	...
	//$consignee['country']  = !empty($consignee['country']) ? $_CFG['shop_country'] : 0;
	//$consignee['province'] = !empty($_CFG['shop_province']) ? $_CFG['shop_province'] : 0;
...
elseif ($_REQUEST['step'] == 'done')
{
	...
	if(!empty($type_money))
	{
		...
		{
			$discount_goods_price = $value['goods_price'];
			//计算商品价格与市场价的比例
			$market_save_rate = $value['market_price'] ? round(($value['market_price'] - $value['goods_price']) * 100 / $value['market_price']) : 0;
			//取得用优惠券后
			if(!empty($type_money) && $type_money>$market_save_rate)
			{
				$value['goods_price'] = $value['market_price']*((100-$type_money)/100);
				$discount_goods_price = $value['goods_price'];
				$market_save_rate = $value['market_price'] ? round(($value['market_price'] - $value['goods_price']) * 100 / $value['market_price']) : 0;
				$cart_goods[$key]['goods_price'] = $value['goods_price'];
				$cart_goods[$key]['formated_goods_price']  = price_format($value['goods_price'], false);
				$value['subtotal'] = $value['goods_price']*$value['goods_number'];
				$cart_goods[$key]['formated_subtotal']     = price_format($value['subtotal'], false);
			}
			$cart_goods[$key]['save_rate'] = $market_save_rate;
			
			$sql = "update " .$GLOBALS['ecs']->table('cart')." set discount_goods_price=$discount_goods_price where rec_id=$value[rec_id]";
            $db->query($sql);
		}
	}	
	else
	...
	//收据信息
	$billing_method  = empty($_POST['billing_method']) ? 1 : intval($_POST['billing_method']);
	
	if($billing_method==2)
	{
		$billing_consignee = array(
			'billing_firstname'     => empty($_POST['billing_firstname'])  ? '' : compile_str(trim($_POST['billing_firstname'])),
			'billing_lastname'     => empty($_POST['billing_lastname'])  ? '' : compile_str(trim($_POST['billing_lastname'])),
			'billing_country'       => empty($_POST['billing_country'])    ? '' : intval($_POST['billing_country']),
			'billing_province'      => empty($_POST['billing_province'])   ? '' : trim($_POST['billing_province']),
			'billing_city'          => empty($_POST['billing_city'])       ? '' : compile_str($_POST['billing_city']),
			'billing_email'         => empty($_POST['billing_email'])      ? '' : compile_str($_POST['billing_email']),
			'billing_address'       => empty($_POST['billing_address'])    ? '' : compile_str($_POST['billing_address']),
			'billing_address_2'       => empty($_POST['billing_address_2'])    ? '' : compile_str($_POST['billing_address_2']),
			'billing_zipcode'       => empty($_POST['billing_zipcode'])    ? '' : compile_str(make_semiangle(trim($_POST['billing_zipcode']))),
			'billing_tel'           => empty($_POST['billing_tel'])        ? '' : compile_str(make_semiangle(trim($_POST['billing_tel']))),
	       );
	}
	else
	{
		...
		{
			$billing_consignee['billing_'.$key] = addslashes($value);
		}
	}
	foreach ($billing_consignee as $key => $value)
    {
   ...
   /* 判断是不是实体商品 */
    ...
    /*--wzys修改过代码--*/
	$order['curr_to'] = $currency_to;
	$order['curr_rate'] = $currency_rate;
	$order['curr_format'] = $currency_format;
    /*--wzys修改过代码 end--*/
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
            ...
            " SELECT '$new_order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, ".
                "discount_goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id".
            " FROM " .$ecs->table('cart') .
	...
	if ($_CFG['is_order_reminded_costome'] && $order['email'] != '')
    {
        $tpl = get_mail_template('order_reminded_costome');
        $smarty->assign('order', $order);
        $smarty->assign('goods_list', $cart_goods);
        $smarty->assign('shop_name', $_CFG['shop_name']);
        $smarty->assign('send_date', date($_CFG['time_format']));
        $content = $smarty->fetch('str:' . $tpl['template_content']);
        send_mail($_CFG['shop_name'], $order['email'], $tpl['template_subject'], $content, $tpl['is_html']);
    }
...
//    die(sprintf($_LANG['bonus_is_ok'], price_format($bonus['type_money'], false)));
    ...
    else
    {
		...
		{
			$result['error'] = "This coupon code will be available only when the order's amount reach ".price_format($bonus['min_goods_amount']).".";
        ...
        $result['content'] = $smarty->fetch('library/order_total.lbi');
		$result['cart_goods'] = $smarty->fetch('library/cart_goods.lbi');
    }
    $json = new JSON();
    die($json->encode($result));
}
