...
class cncard
{
    ...
    {
        $c_mid      = trim($payment['c_mid']);  //商户編號，在申请商户成功后即可获得，可以在申请商户成功的邮件中获取该編號
        $c_order    = $order['order_sn'];       //商户网站依照訂單号规则生成的訂單号，不能重复
        $c_name     = "";                       //商户訂單中的收货人姓名
        $c_address  = "";                       //商户訂單中的收货人地址
        $c_tel      = "";                       //商户訂單中的收货人电话
        $c_post     = "";                       //商户訂單中的收货人邮编
        $c_email    = "";                       //商户訂單中的收货人Email
        $c_orderamount = $order['order_amount'];        //商户訂單总金额
        ...
        //$c_ymd      = substr($order['order_sn'], 0, 8); //商户訂單的产生日期，格式为"yyyymmdd"，如20050102
        $c_moneytype= $payment['c_moneytype'];          //支付币种，0为人民币
        $c_retflag  = "1";                              //商户訂單支付成功后是否需要返回商户指定的文件，0：不用返回 1：需要返回
    ...
    function respond()
    {
        ...
        $c_mid          = $_REQUEST['c_mid'];           //商户編號，在申请商户成功后即可获得，可以在申请商户成功的邮件中获取该編號
        $c_order        = $_REQUEST['c_order'];         //商户提供的訂單号
        $c_orderamount  = $_REQUEST['c_orderamount'];   //商户提供的訂單总金额，以元为单位，小数点后保留两位，如：13.05
        $c_ymd          = $_REQUEST['c_ymd'];           //商户传输过来的訂單产生日期，格式为"yyyymmdd"，如20050102
        $c_transnum     = $_REQUEST['c_transnum'];      //云网支付网关提供的该笔訂單的交易流水号，供日后查询、核对使用；
        $c_succmark     = $_REQUEST['c_succmark'];      //交易成功标志，Y-成功 N-失败
        $c_moneytype    = $_REQUEST['c_moneytype'];     //支付币种，0为人民币
        $c_cause        = $_REQUEST['c_cause'];         //如果訂單支付失败，则该值代表失败原因
        ...
        //--校验商户編號
        $MerchantID= trim($payment['c_mid']);   //商户自己的編號
        if($MerchantID!=$c_mid){
            //echo "提交的商户編號有误";
...
//      $r_orderamount = $row["訂單金额"];  //商户从自己訂單系统获取该值
//      if($r_orderamount!=$c_orderamount){
//          echo "支付金额有误";
//          exit;
//      }

        //--校验商户訂單系统中记录的訂單生成日期和云网支付网关通知信息中的訂單生成日期是否一致
//      $r_ymd = $row["訂單生成日期"];      //商户从自己訂單系统获取该值
//      if($r_ymd!=$c_ymd){
//          echo "訂單时间有误";
//          exit;
//      }
