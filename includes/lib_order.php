...
{
    $sql = 'SELECT s.shipping_id, a.shipping_area_id, s.shipping_code, s.shipping_name, ' .
            ...
            ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.enabled = 1 group by shipping_name  ORDER BY s.shipping_order';
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order)
    {
        $order['formated_goods_amount']   = price_format($order['goods_amount'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $order['formated_discount']       = price_format($order['discount'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $order['formated_tax']            = price_format($order['tax'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $order['formated_shipping_fee']   = price_format($order['shipping_fee'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $order['formated_insure_fee']     = price_format($order['insure_fee'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $order['formated_pay_fee']        = price_format($order['pay_fee'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $order['formated_pack_fee']       = price_format($order['pack_fee'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $order['formated_card_fee']       = price_format($order['card_fee'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $order['formated_total_fee']      = price_format($order['total_fee'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $order['formated_money_paid']     = price_format($order['money_paid'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $order['formated_bonus']          = price_format($order['bonus'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $order['formated_integral_money'] = price_format($order['integral_money'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $order['formated_surplus']        = price_format($order['surplus'], false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
        $order['formated_order_amount']   = price_format(abs($order['order_amount']), false,true,$order['curr_to'],$order['curr_rate'],$order['curr_format']);
...
function cart_goods($type = CART_GENERAL_GOODS)
{
	...
	foreach($arr as $k=>$v)
	{
		$goods_thumb = $GLOBALS['db']->getOne("SELECT `goods_thumb` FROM " . $GLOBALS['ecs']->table('goods') . " WHERE `goods_id`='{$v['goods_id']}'"); 
		$arr[$k]['goods_thumb'] = get_image_path($v['goods_id'], $goods_thumb, true);
	}
	//添加图片代码结束
    ...
    foreach ($arr as $key => $value)
    {
		...
		$arr[$key]['save_rate'] = $value['market_price'] ? round(($value['market_price'] - $value['goods_price']) * 100 / $value['market_price']).'%' : 0;
    }
    return $arr;
}
...
{
	//判断红包的类型
	$sql = "SELECT t.* FROM " . $GLOBALS['ecs']->table('bonus_type') . " AS t WHERE 1 ";
    if ($bonus_id > 0)
	{
		$sql .= "AND t.type_name = '$bonus_id'";
	}
	else
	{
		$sql .= "AND t.type_name = '$bonus_sn'";
	}
	$bonus_info = $GLOBALS['db']->getRow($sql);
	
	
	if($bonus_info['send_type']==4)
	{
       return $bonus_info;
	}
	elseif($bonus_info['send_type']==0)
	{
	   $time = gmtime();
       $type_id = $bonus_info['type_id'];
	   $sql = "SELECT t.*, b.* " .
				"FROM " . $GLOBALS['ecs']->table('bonus_type') . " AS t," .
					$GLOBALS['ecs']->table('user_bonus') . " AS b " .
				"WHERE t.type_id = b.bonus_type_id and b.bonus_type_id='$type_id' and user_id='$_SESSION[user_id]' and b.start_time<='$time' and b.end_time>='$time'";
...
function check_consignee_info($consignee, $flow_type)
{
    ...
    {
        /* 如果存在实体商品 */
        $res = !empty($consignee['firstname']) &&
		!empty($consignee['lastname']) &&
