...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        /*** 严学文，在读取商品信息的时候把商品的小图片: goods_thumb 也读取出来, select加了这一项，不需要的时候可去掉 ***/
		$sql = "SELECT goods_id, goods_name, goods_type, goods_sn, goods_sn1, goods_thumb, shop_price,goods_buy_price,goods_weight,goods_weight_review,is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
...
function reformat_image_name_copy($type, $goods_id, $source_img, $position='')
{
    $rand_name = gmtime() . sprintf("%03d", mt_rand(1,999));
    ...
    {
        case 'goods':
            $img_name = 'copy_'.$goods_id . '_G_' . $rand_name;
            break;
        case 'goods_thumb':
            $img_name = 'copy_'.$goods_id . '_thumb_G_' . $rand_name;
            break;
        case 'gallery':
            $img_name = 'copy_'.$goods_id . '_P_' . $rand_name;
            break;
        case 'gallery_thumb':
            $img_name = 'copy_'.$goods_id . '_thumb_P_' . $rand_name;
            break;
    }
	
    if ($position == 'source')
    {
        if (move_image_file_copy(ROOT_PATH.$source_img, ROOT_PATH.$dir.'/'.$sub_dir.'/source_img/'.$img_name.$img_ext))
        {
            return $dir.'/'.$sub_dir.'/source_img/'.$img_name.$img_ext;
        }
    }
    elseif ($position == 'thumb')
    {
        if (move_image_file_copy(ROOT_PATH.$source_img, ROOT_PATH.$dir.'/'.$sub_dir.'/thumb_img/'.$img_name.$img_ext))
        {
            return $dir.'/'.$sub_dir.'/thumb_img/'.$img_name.$img_ext;
        }
    }
    ...
    {
        if (move_image_file_copy(ROOT_PATH.$source_img, ROOT_PATH.$dir.'/'.$sub_dir.'/goods_img/'.$img_name.$img_ext))
        {
...
function move_image_file_copy($source, $dest)
{
    if (@copy($source, $dest))
    {
        return true;
