...
function get_cat_goods($cat_id=0,$relative_goods_ids,$num=10,$goods_id=0)
{
    
	if(empty($relative_goods_ids))
	{
		$children = get_children($cat_id);
        $where = "g.is_on_sale = 1 AND g.is_alone_sale = 1 and goods_id<>$goods_id AND ".
            "g.is_delete = 0 AND ($children OR " . get_extension_goods($children) . ')';
		$sql = 'SELECT g.goods_id FROM ' . $GLOBALS['ecs']->table('goods') . " AS g  WHERE $where";
		$goods_ids = $GLOBALS['db']->getCol($sql);
		
		$relative_goods_ids = array_rand($goods_ids, $num);
		$relative_goods_ids = implode(',',$relative_goods_ids);
		$sql = " update ".$GLOBALS['ecs']->table('goods')." set relative_goods_ids='$relative_goods_ids' where goods_id='".$goods_id."'";
	    $GLOBALS['db']->query($sql);
	}
	
	$relative_goods_ids = explode(',',$relative_goods_ids);
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
            ...
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
            "WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0  and g.goods_id " . db_create_in($relative_goods_ids) ." ORDER BY goods_id DESC limit $num";
