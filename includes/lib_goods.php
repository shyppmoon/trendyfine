...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
                "g.goods_attr_id, g.attr_value, g.attr_price, a.comstome_type " .
	...
	/*wzys属性改进代码*/
        if ($row['comstome_type'] == 1)
        {
			
			//判断是否有为存
			$sql = "SELECT product_number FROM " . $GLOBALS['ecs']->table('products') . "  as g WHERE g.goods_id='$goods_id' AND goods_attr='".$row['goods_attr_id']."'";
        ...
        else
        {
			...
			$arr['pro'][$group][$row['attr_id']]['value'] = trim($arr['pro'][$group][$row['attr_id']]['value'],',');
			$arr['pro'][$group][$row['attr_id']]['show_in_desc'] = $row['show_in_desc'];
		}
